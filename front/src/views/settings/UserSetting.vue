...
import {authHeader, get, post} from "@/net";
  ...
  baseFormRef.value.validate(isValid => {
    if (isValid) {
      ...
      post('/api/user/save-details', baseForm, () => {
        ...
        describe.value = baseForm.describe
        loading.base = false
      }, (message) => {
...
get('/api/user/details', data => {
  ...
  baseForm.qq = data.qq
  baseForm.wx = data.wx
  baseForm.describe= describe.value = data.describe
  loading.form = false
})
...
function beforeAvatarUpload(rawFile) {
  if (rawFile.type !== 'image/jpeg' && rawFile.type !== 'image/png') {
    ...
    ElMessage.error('头像大小不能大于 200KB')
    return false
  }
  return true
}
      ...
      <card :icon="User" title="账号信息设置" desc="在这里编辑您的个人信息，您可以在隐私设置中选择是否展示这些信息"
        ...
        <el-form :model="baseForm" :rules="rules"  ref="baseFormRef" label-position="top"
          ...
          <el-form-item label="个人简介" prop="desc">
            <el-input v-model="baseForm.describe" type="textarea" :rows="6" maxlength="200"/>
          </el-form-item>
          <div>
