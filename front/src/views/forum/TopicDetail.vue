...
import User from '@/components/UserPriview.vue';
...
function loadComments(page) {
  topic.comments = null
  topic.page = page
  get(`/api/forum/comment?tid=${tid}&page=${page - 1}`, data => topic.comments = data)
}
...
// 抽屉打开时的初始化函数
const initEditor = () => {
  console.log('抽屉已打开，进行初始化操作');
};

// 关闭抽屉时触发的事件
const emitCloseEvent = () => {
  console.log('抽屉已关闭');
  show.value = false;  // 关闭抽屉
};
  ...
  <div class="topic-page" v-if="topic.data">
    <div class="topic-main" style="position: sticky;top: 0;z-index: 10">
      <card style="display: flex;width: 100%;">
        <el-button :icon="ArrowLeft" size="large" round style="margin-top: 5px"
        ...
        <button  @click="show = true" style="border: none; background: transparent;">
          <el-avatar :src="store.avatarUserUrl(topic.data.user.avatar)" :size="40"/>
        </button>
          <div style="margin-top: 5px">
            ...
            <div style="font-size: 12px; color: rgba(169, 169, 169, 0.6);">
    ...
    <div class="topic-main">
      <div>
        <!-- 抽屉组件 -->
        <el-drawer :model-value="show" direction="ltr" @open="initEditor"
